...
banner "which package manager to use"
echo "find package manager acodrding to your distro"
echo "debian,ubantu and most use --apt--"
pack="apt"
read -p "Is this debian or ubantu" yn
case $yn in
        y )
                banner "apt is going to use"
                ;;
        * ) 	echo "find package manager according to distro"
		banner "add package manager"
		read -p "add hear -> " pack
		;;
esac
...
sleep 1
sudo $pack install i3 kitty git maim xclip xdotool -y  # maim xclip and xdotool for screenshot
...
sleep 1
sudo $pack install stow polybar rofi neovim nitrogen ranger -y # ranger file manager
pbar 52 1
banner "comman softare"
sleep 1
sudo $pack-get install software-properties-common cmake -y
pbar 56 1
banner "python3 pip"
sudo $pack install python3-pip -y
pbar 60 1
sudo apt install python3-i3ipc
# extra all commented
banner "temp sensore"
sleep 1
sudo $pack install lm-sensors -y
...
read -p "install xinit(to run desktop environment)??? (Y/N)" yn
...
#pbar 1
#pbar 100 1
#make xinitrc file and add in stow for better working
